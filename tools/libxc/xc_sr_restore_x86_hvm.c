                                 ...
                                 const uint32_t *types)
{
    ...
    xen_pfn_t idx1G, idx1G_prev, idx2M, idx2M_prev;
    unsigned i, order;
    int rc = -1;
    ...
    max_pfn = original_pfns[0];
    idx1G_prev = original_pfns[0] >> SUPERPAGE_1GB_SHIFT;
    idx2M_prev = original_pfns[0] >> SUPERPAGE_2MB_SHIFT;
    ...
    for ( i = 0; i < count; ++i )
    {
        ...
        if ( idx1G_prev == idx1G && idx2M_prev == idx2M )
        {
            /* Record highest pfn within this 2MB superpage */
            if ( pfn > max_pfn )
                max_pfn = pfn;
        } else {
            /*
             * If this pfn is in another 2MB superpage it is required to punch
             * holes to release memory, starting from the 1GB boundary up to the
             * highest pfn within the previous 2MB superpage.
             */
            if ( idx1G_prev != idx1G || idx2M_prev != idx2M )
            {
                /*
                 ...
                 * might be holes between max_pfn and the end of the superpage.
                 */
                if ( idx1G_prev != idx1G )
                {
                    order = SUPERPAGE_1GB_SHIFT;
            ...
            /* This is the highest pfn within the next 2MB superpage */
            max_pfn = pfn;
        }
